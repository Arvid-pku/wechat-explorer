  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 4 }}>
        ...
        <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v: number) => fmt(v)} />
        <Legend verticalAlign="top" iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
        <Line type="monotone" dataKey="a" name={seriesLabels[0]} stroke="var(--color-primary)" strokeWidth={2} dot={false} isAnimationActive={false} />
        <Line type="monotone" dataKey="b" name={seriesLabels[1]} stroke="var(--color-muted-foreground)" strokeWidth={1.5} dot={false} isAnimationActive={false} />
      </LineChart>
    </ResponsiveContainer>
  );
}

// ───── Multi-series line chart (one line per series key) ──────────────────
/**
 * Generic multi-line chart for "top N chats over time" style views.
 * `data` is pivoted: each entry has a `label` for the x-axis and one numeric
 * property per series key. `series` defines which keys to plot and what label
